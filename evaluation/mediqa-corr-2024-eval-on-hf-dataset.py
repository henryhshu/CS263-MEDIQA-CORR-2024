...
def parse_reference_file(filepath):
    ...
    for index, row in df.iterrows():
        ...
        reference_flags[text_id] = row['Error Flag']
        reference_sent_id[text_id] = row['Error Sentence ID']

    return reference_corrections, reference_flags, reference_sent_id

DATSET_NAME = "mkieffer/MEDEC"

def parse_reference_dataset(dataset):
    """Parsing reference dataset.
...
class NLGMetrics(object):
    ...
    def compute(self, references, predictions, counters):
        ...
        if 'BLEURT' in self.metrics:
            bleurtscorer = bleurtscore.BleurtScorer(checkpoint=BLEURT_CKPT_PATH)
...
# reference_csv_file = "/home/heewon/workspaces/courses/cs263nlp-26w/MEDEC/MEDEC-MS/MEDEC-MS-ValidationSet-with-GroundTruth-and-ErrorType.csv"
# # reference_csv_file = "data/MEDIQA-CORR-2024-MS-ValidationSet-1.csv"
# # submission_file = "/home/heewon/workspaces/courses/cs263nlp-26w/Project/outputs/medec-ms_gpt5_results_20260203_191635.txt"
# # reference_csv_file = "/home/heewon/workspaces/courses/cs263nlp-26w/MEDEC/MEDEC-MS/MEDEC-MS-TestSet-with-GroundTruth-and-ErrorType.csv"
# submission_file = "/home/heewon/workspaces/courses/cs263nlp-26w/Project/outputs/medec-ms_gpt5_results_20260203_191635.txt"
# reference_csv_file = "/home/heewon/workspaces/courses/cs263nlp-26w/MEDEC/MEDEC-MS/MEDEC-MS-TestSet-50samples-with-GroundTruth-and-ErrorType.csv"

# submission_file = "/home/heewon/workspaces/courses/cs263nlp-26w/Project/outputs/medec-ms_gpt4_1_results_20260205_180349.txt"
# reference_csv_file = "/home/heewon/workspaces/courses/cs263nlp-26w/MEDEC/MEDEC-MS/MEDEC-MS-TestSet-50samples-with-GroundTruth-and-ErrorType.csv"

# submission_file = "/home/heewon/workspaces/courses/cs263nlp-26w/Project/outputs/medec-ms_gpt5_results_20260203_191635.txt"
# submission_file = "/home/heewon/workspaces/courses/cs263nlp-26w/Project/outputs/medec-ms_gpt4_1_results_20260205_180349.txt"
# #reference_corrections, reference_flags, reference_sent_id = parse_reference_file(reference_csv_file)
# submission_file = "/home/heewon/workspaces/courses/cs263nlp-26w/CS263-MEDIQA-CORR-2024/multi-agent/outputs/medec-ms_gpt-5.2_multi-agent_test_20260206_152335.txt"
# submission_file = "/home/heewon/workspaces/courses/cs263nlp-26w/CS263-MEDIQA-CORR-2024/multi-agent/outputs/medec-ms_gpt-4.1_multi-agent_test_20260206_153637.txt"
# split = "test"
...
# reference_corrections, reference_flags, reference_sent_id = parse_reference_dataset(subset_test)

def parse_args():
    parser = argparse.ArgumentParser(description="MEDEC evaluation runner")
    parser.add_argument(
        "--submission_file",
        type=str,
        required=True,
        help="Path to the submission .txt file",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        ...
        required=False,
        default=None,
        help="Path to sampled indices JSON (e.g., sampled_test_indices.json)",
    )
...
if args.indices_file is not None:
    loaded_indices = load_indices(args.indices_file)
    dataset_test = dataset_test.select(loaded_indices)

num_data = len(dataset_test)
reference_corrections, reference_flags, reference_sent_id = parse_reference_dataset(dataset_test)
...
results = { ... }  # your dict
total_results = accuracy_results | nlg_eval_results

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
out_dir = Path("outputs")
out_dir.mkdir(parents=True, exist_ok=True)
out_path = out_dir / f"mediqa-corr-eval-{timestamp}-total.csv"
total_results["submission_file"] = submission_file
total_results["split"] = split
total_results["indices_file"] = args.indices_file if args.indices_file is not None else "None"

with out_path.open("w", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=total_results.keys())
    writer.writeheader()
    writer.writerow(total_results)


out_path2 = out_dir / f"mediqa-corr-eval-{timestamp}-essential.csv"

essential_results = {}
essential_results["submission_file"] = submission_file
essential_results["Error Flags Accuracy"] = accuracy_results["Error Flags Accuracy"]
essential_results["Error Sentence Detection Accuracy"] = accuracy_results["Error Sentence Detection Accuracy"]

essential_results["ROUGE1"] = nlg_eval_results["R1F_subset_check"]
essential_results["BERTSCORE"] = nlg_eval_results["BERTSCORE_subset_check"]
essential_results["BLEURT"] = nlg_eval_results["BLEURT_subset_check"]
essential_results["AggregateComposite"] = nlg_eval_results["AggregateC"]
essential_results["AggregateScore"] = nlg_eval_results["aggregate_subset_check"]

essential_results = {
    k: round(float(essential_results[k]), 4) if isinstance(essential_results[k], np.floating) else essential_results[k]
    for k in essential_results.keys()  
}

with out_path2.open("w", encoding="utf-8") as f:
    writer = csv.DictWriter(f, fieldnames=essential_results.keys())
    writer.writeheader()
    writer.writerow(essential_results)
