...
CRITIC_INSTRUCTIONS = """You are a clinical-text consistency critic for a benchmark task.
You must NOT provide clinical advice.
...
4) Risk control: Reject changes that introduce risky new clinical actions (e.g., new drug/dose/diagnosis) without support in the note.
5) Wrong-type fix: Determine which error type the proposal attempts to fix; reject proposals that fix the wrong type or change unrelated clinical content.
